...
class MessageModerator {
    ...
    execute(user: User, packet: SocketMessage, universeManager: UniverseManager, serverManager: ServerManager): void {
        if (!user.hasRight('MSGMAP')) return
        ...
        for (const camera of serverManager.getListCamera()) {
            ...
            if ((!processedUserPidsByMap[serverId][currMap].has(camera.user.pid)) && camera.currMap === mapId && (isAllUnivers || camera.serverId === serverIdMsg)) {
